...
import 'package:orders/String/place_order_string.dart';
...
import 'package:orders/state/place_order_state.dart';
import 'package:orders/utils/const.dart';
import 'package:orders/view-model/process_order/proces_order_vm_imp.dart';
...
class PlaceOrderScreen extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    Scaffold(
      ...
      appBar: AppBar(title: Text(placeOrderText),),
      body: Form(key: formKey, child: Padding(
            ...
            Row(
              children: [
                Expanded(child: TextFormField(
                  ...
                  decoration: InputDecoration(hintText: firstNameText,label: Text(firstNameText)
                ...
                Expanded(child: TextFormField(
                  ...
                  decoration: InputDecoration(hintText: lastNameText,label: Text(lastNameText)
                      ,border: OutlineInputBorder()),
                ),),
              ],
            ),
            ...
            Column(
              ...
              children: [
                TextFormField( controller: addressController,
                  validator:ValidationBuilder(requiredMessage:'$addressText $isRequiredText' ).required().build(),
                  decoration: InputDecoration(hintText: addressText,label: Text(addressText),border: OutlineInputBorder()),)
              ],
            ),
            SizedBox(height: 8,),
            Obx(()=>  Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(paymentText,style: GoogleFonts.jetBrainsMono(fontSize: 22,fontWeight: FontWeight.bold),
                  textAlign: TextAlign.left, ),
                RadioListTile <String>(
                    title:  const Text(codText),
                    value: COD_VAL, groupValue: placeOrderState.paymentSelected.value, onChanged: (String? val){
                  placeOrderState.paymentSelected.value=val!;
                }),
                RadioListTile <String>(
                    title:  const Text(braintreeText),
                    value: BRAINTREE_VAL, groupValue: placeOrderState.paymentSelected.value, onChanged: (String? val){
                  placeOrderState.paymentSelected.value=val!;
                }),
                RadioListTile <String>(
                    title:  const Text(paypalText),
                    value: PAYPAL_VAL, groupValue: placeOrderState.paymentSelected.value, onChanged: (String? val){
                  placeOrderState.paymentSelected.value=val!;
                }),
            ...
            Column(
              ...
              children: [
                TextFormField( controller: commentController,
                  validator:ValidationBuilder(requiredMessage:'$commentText $isRequiredText' ).required().build(),

                  decoration: InputDecoration(hintText: commentText,label: Text(commentText),border: OutlineInputBorder()),)
              ],
            ),
            Spacer(),
            Container(
              width: double.infinity,
              child: ElevatedButton(child: Text(placeOrderText),
                onPressed: ()async{
